...
pub trait Formula<I, O>: Debug + Clone + Send + Sync {
...
pub trait IntoLineResult: Send + Sync {
...
impl<O: Display + Send + Sync> IntoLineResult for &O {
    fn into_line_result(self) -> LineResult {
...
impl<I: Copy, O: IntoLineResult, F: Clone + Send + Sync + Fn(I) -> O> Formula<I, O> for Opaque<F> {
    fn compute(&self, input: I) -> O {
        (self.fun)(input)
    }
    fn description<D: Descriptor>(&self, desc: &mut D, input: Option<I>) -> fmt::Result {
...
pub fn opaque<I: Copy, O: IntoLineResult, F: Clone + Send + Sync + Fn(I) -> O>(
    name: &'static str,
    fun: F,
) -> Op<I, O, Opaque<F>> {
    op(Opaque { name, fun })
