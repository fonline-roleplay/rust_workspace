...
#[derive(Copy)]
pub struct OpPhantomData<T>(PhantomData<*const T>);
impl<T> OpPhantomData<T> {
    pub const fn new() -> Self {
        Self(PhantomData)
    }
}
unsafe impl<T> Send for OpPhantomData<T> {}
unsafe impl<T> Sync for OpPhantomData<T> {}
impl<T> Debug for OpPhantomData<T> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        f.debug_tuple("OpPhantomData").finish()
    }
}
impl<T> Clone for OpPhantomData<T> {
    fn clone(&self) -> Self {
        Self::new()
    }
}
...
pub struct Op<I, O, F: Formula<I, O>>(
    pub F,
    pub(super) OpPhantomData<I>,
    pub(super) OpPhantomData<O>,
);
...
pub fn op<I, O, F: Formula<I, O>>(f: F) -> Op<I, O, F> {
    Op(f, OpPhantomData::new(), OpPhantomData::new())
}
