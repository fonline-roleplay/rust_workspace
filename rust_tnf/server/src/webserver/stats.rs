...
const STAT_TITLES: [&'static str; 10] = ["Гадко","Плохо","Низко","Неплохо","Средне","Хорошо","Высоко","Отлично","Круто","Герой!"];
const STAT_NAMES: [&'static str; 7] = ["СЛ","ВC","ВН","ОБ","ИН","ЛВ","УД"];

const SKILL_NAMES: [&'static str; 18] = ["Легкое оружие",
    "Тяжелое оружие",
    "Энергооружие",
    "Рукопашная",
    "Xолодное оружие",
...
impl<'a> Stats<'a> {
    fn new(cr: &'a CritterInfo) -> Self {
        ...
        let stat_fields = slice.iter().enumerate().map(|(index, &st)| {
        ...
        let skill_fields = slice.iter().enumerate().map(|(index, &sk)| {
            SkillField{
                name: SKILL_NAMES[index],
                value: sk.max(0).min(999),
                tagged: tagged.iter().any(|tag| *tag as i32 == Param::SK_SMALL_GUNS as i32 + index as i32)
            }
        }).collect();
        ...
        dbg!(Stats {
            ...
            sex: if cr.param(Param::ST_GENDER)==0 {"МУЖ."} else {"ЖЕН."},
            level,
